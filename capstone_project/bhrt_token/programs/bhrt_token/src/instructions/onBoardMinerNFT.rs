...
pub struct OnboardMinerNFT<'info> {
  #[account(mut)]
...
impl<'info> OnboardMinerNFT<'info> {


     pub fn onboard_miner_nft( &mut self, nft_id: u64,name: String, uri: String, bump: &OnboardMinerNFTBumps) -> Result<()> {
...
let signer_seeds = &[&state_seeds[..]];
        ...
        // VerifyCollectionV1CpiBuilder::new(&self.metadata_program.to_account_info())
        //     .authority(&self.program_state.to_account_info())
        //     .metadata(&self.miner_nft_metadata.to_account_info())
        //     .collection_mint(&self.collection_mint.to_account_info())
        //     .collection_metadata(Some(&self.nft_collection_metadata.to_account_info()))
        //     .collection_master_edition(Some(&self.collection_master_edition_account.to_account_info()))
        //     .system_program(&self.system_program.to_account_info())
        //     .sysvar_instructions(&self.instruction_sysvar.to_account_info())
        //     .invoke_signed(signer_seeds)?;
...
self.miner_info.set_inner(MinerInfo {
    ...
    hashrate_token_mint: Pubkey::default(),
    mint_amount: 0,
